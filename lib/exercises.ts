import type { ExerciseDef, ExerciseDetail, ExerciseMedia } from "./types";
import mediaData from "./exercise-media.generated.json";
...
// ── Animated GIFs + extra muscle data (hasaneyldrm dataset) ─────────────────
// Generated by scripts/build-exercises.mjs. Keyed by canonical display name.
const MEDIA: Record<string, ExerciseMedia> = (mediaData as { exercises: Record<string, ExerciseMedia> })
  .exercises;

/** Dataset enrichment (gif + secondary muscles) for an exercise, if known. */
export function mediaFor(name: string): ExerciseMedia | undefined {
  return MEDIA[name];
}

/** Animated-GIF URL for an exercise, or null (UI falls back to photo/glyph). */
export function gifUrlFor(name: string | null | undefined): string | null {
  if (!name) return null;
  return MEDIA[name]?.gifUrl ?? null;
}

/** Title-case a lowercase dataset muscle/equipment token for display. */
export function titleCase(s: string): string {
  return s.replace(/\b\w/g, (c) => c.toUpperCase());
}

/** Human descriptor for an equipment type, e.g. "Barbell, free weight". */
export function equipmentLabel(equipment: string): string {
  const e = equipment.toLowerCase();
  if (e === "barbell" || e === "dumbbell" || e === "kettlebell") {
    return `${titleCase(equipment)}, free weight`;
  }
  if (e === "cable") return "Cable, pulley";
  if (e === "machine") return "Machine, selectorized";
  if (e === "bodyweight") return "Bodyweight, no equipment";
  return titleCase(equipment);
}

/**
 * Resolve everything the /exercise/[name] detail modal needs. Combines the
 * curated library (primary muscle, equipment, photo slug) with the dataset
 * enrichment (gif, secondary muscles). Returns null for unknown names.
 */
export function getExerciseDetail(name: string): ExerciseDetail | null {
  const def = BY_NAME.get(name);
  const media = MEDIA[name];
  if (!def && !media) return null;
  return {
    name,
    slug: def?.slug ?? null,
    primaryMuscle: def?.primaryMuscle ?? (media ? titleCase(media.target) : ""),
